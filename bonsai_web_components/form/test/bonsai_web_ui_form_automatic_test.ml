...
let%expect_test "collapsible group" =
  let component graph =
    ...
    in
    Form.Dynamic.collapsible_group (Bonsai.return "group-name") textbox graph
  in
        ...
        <tr>
          <td colspan="2" style={ padding-left: 0em; font-weight: bold; }>
            <label style={ display: block; }>
              <div @on_click style={ user-select: none; cursor: pointer; }> ▾ group-name </div>
            </label>
          </td>
        </tr>
...
let%expect_test "using the same component twice" =
  let component graph =
    let textbox =
      Form.Elements.Textbox.string ~allow_updates_when_focused:`Never () graph
    in
    Bonsai.map2 ~f:Form.both textbox textbox
  in
  let handle =
    Handle.create
      (form_result_spec
  ...
  [%expect
    ...
    <table>
      <tbody>
        <tr @key=bonsai_path_x/> <- [ERRORS]: Siblings have same vdom key
        <tr @key=bonsai_path_x/> <- [ERRORS]: Siblings have same vdom key
      </tbody>
    </table>
...
let%expect_test "form of nested record of int and float" =
  let module T = struct
    module Nested = struct
      ...
      let form graph =
        ...
        Form.Dynamic.Record_builder.(
          build_for_record
            (Fields.make_creator ~age:(field age) ~height:(field height))
            graph)
      ;;
    end
    ...
    let form graph =
      ...
      Form.Dynamic.Record_builder.(
        build_for_record
          (Fields.make_creator ~unit:(field unit) ~nested:(field nested))
          graph)
    ;;
  end
  in
...
let%expect_test "typed records: dynamic labelling" =
  let module T = struct
    ...
    let form (int_label : string Bonsai.t) =
      Form.Typed.Record.make
        (module struct
          module Typed_field = Typed_field
  ...
  in
  let label = Bonsai.Expert.Var.create "initial label" in
  let handle =
    Handle.create
      (form_result_spec
         ~filter_printed_attributes:(fun ~key:_ ~data:_ -> false)
         ~get_vdom:get_vdom_verbose
         [%sexp_of: T.t])
      (T.form (Bonsai.Expert.Var.value label))
  in
  Handle.show handle;
  [%expect
    {|
    ...
    |}];
  Bonsai.Expert.Var.set label "updated label";
  Handle.show_diff handle;
  [%expect
    {|
      (Ok ((int 0)))
...
let%expect_test "typed variants: attr is applied to dropdown" =
  let module T = struct
    ...
    let form =
      Form.Typed.Variant.make
        ~picker_attr:(Bonsai.return (Vdom.Attr.class_ "foo"))
        (module struct
          module Typed_variant = Typed_variant
      ...
      Form.Typed.Variant.make
        ...
        (module struct
          ...
          let form_for_variant
            ...
            | String ->
              Form.Elements.Textbox.string ~allow_updates_when_focused:`Never () graph
            | Other_thing -> Bonsai.return (Form.return ())
          ;;
        end)
    ;;
  end
...
let%expect_test "file picker list" =
  ...
  [%expect
    ...
    ("WARNING: Attempted to set the value of a file select to a value other than [Map.empty]. This is prohibited by the browser and therefore ignored."
     (files ((bar.pdf "<file bar.pdf>") (foo.txt "<file foo.txt>"))))
    |}]
;;
    ...
    <form @on_submit>
      <table>
        <tbody>
          ...
          <tr>
            <td>
              <button class="my-class" @on_click> submit </button>
            </td>
          </tr>
        </tbody>
      </table>
...
let%expect_test "difference between with_default and with_default_always" =
  ...
  let create_handle ~name with_default =
    let component graph =
      ...
      | false -> Bonsai.return (Form.return 0)
    in
    Handle.create
      (Result_spec.string
         (module struct
...
let%expect_test "Partially settable form via [Form.return] and [Form.return_settable]" =
  let module Record = struct
    ...
    let form_for_field : type a. a Typed_field.t -> Bonsai.graph -> a Form.t Bonsai.t =
      ...
      | Another_form_return -> Bonsai.return (Form.return ~sexp_of_t:[%sexp_of: int] 3)
    ;;
  ...
  let module M = struct
    ...
    let form_for_variant
      ...
      | Foo ->
        Form.Elements.Checkbox.set
          comparator
          ~extra_container_attrs:(Bonsai.return [ Vdom.Attr.class_ "foo" ])
          (Bonsai.return Foo.all)
      | Bar ->
        Form.Elements.Checkbox.set
          comparator
          ~extra_container_attrs:(Bonsai.return [ Vdom.Attr.class_ "bar" ])
          (Bonsai.return Bar.all)
    ;;
