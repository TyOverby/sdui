...
module Test_read_on_change = struct
  let%expect_test "create_single" =
    ...
    Bonsai.Expert.Var.set t_var (Some (For_testing.create stream_data));
    show ();
  ...
  let%expect_test "create_multiple" =
    ...
    Bonsai.Expert.Var.set ts_var (map [ file1; file2; file3 ]);
    show ();
    [%expect
      {|
    ...
    [%expect
      ...
      |}];
    Bonsai.Expert.Var.set ts_var (map [ file1; file2 ]);
    show ();
    [%expect
      {|
      ((foo1.txt (In_progress ((loaded 0) (total 11))))
    ...
    Bonsai.Expert.Var.set ts_var (map [ file1; file2; file4 ]);
    show ();
    [%expect
      {|
      ((foo1.txt (In_progress ((loaded 0) (total 11))))
    ...
    Bonsai.Expert.Var.set ts_var (map [ file1_new; file2; file4 ]);
    show ();
    [%expect
      {|
