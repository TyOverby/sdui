...
module Action = struct
  ...
  let map_target : ('source, 'a) t -> f:('a -> 'b) -> ('source, 'b) t =
    ...
    | Started_drag { source; offset; position; size; target } ->
      Started_drag { source; offset; position; size; target = Option.map target ~f }
  ;;
end
...
let add_event_listener, remove_event_listener =
  let active : Dom_html.event_listener_id Bonsai_private.Path.Map.t ref =
    ref Bonsai_private.Path.Map.empty
  in
  let install =
      ...
      ~apply_action:(fun context on_drop model actions ->
        ...
        | Active on_drop ->
          List.fold actions ~init:model ~f:(fun model action ->
            match action with
            | Action.Started_drag { source; offset; position; size; target } ->
              (match model with
               | State_machine_model.Not_dragging -> ()
               | Dragging _ -> bug "Started dragging before dragging finished");
              Dragging { source; offset; position; size; target; has_moved = false }
            | Set_target target ->
              (match model with
            ...
            | Finished_drag ->
              (match model with
               | State_machine_model.Not_dragging
               | Dragging { target = None; _ }
               | Dragging { has_moved = false; _ } -> Not_dragging
               | Dragging { source; target = Some target; position; _ } ->
                 Bonsai.Apply_action_context.schedule_event
                   context
                   (on_drop position source target);
                 Not_dragging)
    ...
    fun ~id ->
      Vdom.Attr.many
        [ Vdom.Attr.on_pointerdown (fun event ->
            ...
            | true -> Effect.Ignore
            | false ->
              inject [ Started_drag { source = id; offset; position; size; target } ])
        ; Style.no_select
        ]
  in
...
let create
  ...
  create_with_drop_position
    ...
    ~target_id:(module Target)
    ~on_drop:(on_drop >>| fun f (_ : Position.t) a b -> f a b)
;;

let dragged_element t ~f graph =
  match%sub t >>| model with
  | Not_dragging | Dragging { has_moved = false; _ } -> Bonsai.return Vdom.Node.None
