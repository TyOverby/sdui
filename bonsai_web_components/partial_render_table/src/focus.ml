...
module By_cell = struct
  ...
  type ('k, 'column_id, 'presence) t =
    ...
    ; focus_top : unit Effect.t
    ; focus_bottom : unit Effect.t
    ; focus_leftmost : unit Effect.t
  ...
  module Statically_computed : sig
    ...
    let create inject =
      ...
      ; focus_top = inject Top
      ; focus_bottom = inject Bottom
      ; focus_leftmost = inject Leftmost
...
module By_row = struct
  ...
  (* It's important that we not curry these, because that breaks phys-equality, and
     results in unnecessary recomputations. *)
  let focus = By_cell.focus
  let focus_index = By_cell.focus_index
...
module Kind = struct
  type ('a, 'presence, 'k, 'column_id) t =
    ...
    | By_cell :
        { on_change : (('k * 'column_id) option -> unit Effect.t) Bonsai.t
        ; compute_presence :
            ('k * 'column_id) option Bonsai.t -> Bonsai.graph -> 'presence Bonsai.t
        ; key_rank : ('k -> int option Effect.t) Bonsai.t
        }
...
module Cell_machine = struct
  ...
  let component
    ...
    ~(compute_presence :
        (key * column_id) option Bonsai.t -> Bonsai.graph -> presence Bonsai.t)
    ...
    : Bonsai.graph -> ((key, column_id, presence) By_cell.t, key, column_id) t Bonsai.t
    =
    fun graph ->
             ...
             | Rightmost ->
               ( Visible
                   (update_focus ~f:(function
                     ...
                     | None -> range_start, List.last_exn columns))
               , `Cancel )
             | Page_down ->
               ( Visible
                   (update_focus ~f:(function
...
module Row_machine = struct
  let component
    ...
    (key : (key, cmp) Bonsai.comparator)
    ~(compute_presence : key option Bonsai.t -> Bonsai.graph -> presence Bonsai.t)
    ~(on_change : (key option -> unit Effect.t) Bonsai.t)
    ~(key_rank : (key -> int option Effect.t) Bonsai.t)
    ~(collated : (key, data) Incr_map_collate.Collated.t Bonsai.t)
    ~(range : (int * int) Bonsai.t)
    ~(scroll_to_index : (int -> unit Effect.t) Bonsai.t)
    : Bonsai.graph -> ((key, presence) By_row.t, key, _) t Bonsai.t
    =
    fun graph ->
...
let component
  ...
  match kind with
  | None ->
    fun _ _ ~collated:_ ~leaves:_ ~range:_ ~scroll_to_index:_ ~scroll_to_column:_ _graph ->
      Bonsai.return { focus = (); visually_focused = Nothing_focused }
  | By_row { on_change; compute_presence; key_rank } ->
    fun key _ ~collated ~leaves:_ ~range ~scroll_to_index ~scroll_to_column:_ graph ->
...
let get_on_cell_click
  ...
  (value : r Bonsai.t)
  : (k -> column_id -> unit Effect.t) Bonsai.t
  =
  ...
  match kind with
  | None -> Bonsai.return (fun _ _ -> Effect.Ignore)
  | By_row _ ->
    let focus =
