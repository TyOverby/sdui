...
let table_form
  ...
  in
  let id = Bonsai.map (Bonsai.path graph) ~f:Bonsai.Path.to_unique_identifier_string in
  let map, set_map =
    Bonsai.state
  ...
  in
  let%sub forms, table =
    let forms = Bonsai.assoc (module Key) map ~f:(fun _key _data -> form_of_t) graph in
    let%sub { view; _ } =
      let columns = C.lift columns in
      Table.component
        (module Key)
        ~focus:None
        ~row_height:(Bonsai.return (`Px 25))
        ~columns
        forms
        graph
    in
