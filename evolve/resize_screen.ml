...
let component (local_ graph) =
  ...
  in
  let scale_factor =
    Form.Elements.Number.float ~min:0.1 ~max:10.0 ~default:1.0 ~step:0.01 () graph
  in
  let keep_area =
    ...
    Effect.lazy_
      (lazy
        (let%bind.Effect image =
         ...
         in
         let old_width = Canvas2d.Image.width image in
         let old_height = Canvas2d.Image.height image in
         let scale_factor = Form.value_or_default ~default:1.0 scale_factor in
         let new_width =
           nearest_multiple_of_8 (Float.to_int (Float.of_int old_width *. scale_factor))
         in
         let new_height =
           nearest_multiple_of_8 (Float.to_int (Float.of_int old_height *. scale_factor))
         in
         let%bind.Effect canvas_image =
           Sd.Load_image_effect.load_image_generic
             (Canvas2d.Image.resize image ~w:new_width ~h:new_height)
         in
         let image =
           Sd.Image.of_string ~kind:Base64 (Canvas2d.Image.to_data_url canvas_image)
         in
  ...
  fun ~get_images ~set_result ->
    let padding =
      let on_click =
        ...
        let%bind.Effect ~new_width, ~new_height, ~old_width, ~old_height, ~image:output =
          add_padding_effect input.Sd_chain.Paint.Images.image
        in
      ...
      View.card'
        ...
        [ View.vbox
            ...
            ; Form.view keep_area
            ; View.button theme "add padding" ~on_click
            ]
        ]
    in
    let resize =
      let on_click =
        let%bind.Effect input = get_images in
        let%bind.Effect ~new_width, ~new_height, ~image:output =
          scale_effect input.Sd_chain.Paint.Images.image
        in
        set_result
          ~new_width:(Int63.of_int new_width)
          ~new_height:(Int63.of_int new_height)
      ...
      View.card'
        ...
        [ View.vbox [ Form.view scale_factor; View.button theme "Resize" ~on_click ] ]
    in
    View.card'
      theme
      ~title_kind:View.Constants.Card_title_kind.Discreet
      ~title:[ Vdom.Node.text "Resize" ]
      [ View.hbox [ padding; resize ] ]
;;
